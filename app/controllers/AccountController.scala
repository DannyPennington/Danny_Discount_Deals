...
class AccountController @Inject()(
  ...
  def deleteUser(): Action[AnyContent] = Action.async { implicit request: Request[AnyContent] =>
    val user = getAccount(request.session.get("user").getOrElse(""))
    val futureDelete = collection.flatMap(_.delete.one(user))
    futureDelete.map(_ => Redirect(routes.HomeController.index()).withSession("delete" -> "yes"))
  }

  def confirmDelete(): Action[AnyContent] = Action { implicit request: Request[AnyContent] =>
    Ok(views.html.confirmdelete())
  }
}